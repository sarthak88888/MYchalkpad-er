...
export default function TransferCertificatesScreen() {
  ...
  async function handleGenerateTC() {
    ...
    if (!tcForm.leaving_date.trim()) { Alert.alert('Error', 'Please enter leaving date.'); return; }
    if (!tcForm.reason.trim()) { Alert.alert('Error', 'Please enter reason for leaving.'); return; }
    setSaving(true);
    try {
      ...
      setShowForm(false); setSelectedStudent(null); setSearchQuery('');
  ...
  function renderTCCard({ item }: { item: TransferCertificate }) {
    return (
      <TouchableOpacity style={styles.card} onPress={() => { setSelectedTC(item); setShowDetailModal(true); }} activeOpacity={0.8}>
        <View style={styles.cardTop}>
          ...
          <View style={[styles.statusBadge, { backgroundColor: item.status === 'collected' ? COLORS.success + '18' : COLORS.warning + '18' }]}>
            <Text style={[styles.statusText, { color: item.status === 'collected' ? COLORS.success : COLORS.warning }]}>
      ...
      <ScrollView style={styles.scroll} keyboardShouldPersistTaps="handled">
        <View style={styles.searchSection}>
          ...
          <View style={styles.searchRow}>
            ...
            <TextInput style={styles.searchInput} placeholder="Search student by name, roll, class..." placeholderTextColor={COLORS.textSecondary} value={searchQuery} onChangeText={handleSearch} />
          ...
          {searchResults.length > 0 && (
            <View style={styles.searchDropdown}>
              {searchResults.map(student => (
                <TouchableOpacity key={student.id} style={styles.searchResultItem} onPress={() => selectStudent(student)}>
                  <Text style={styles.searchResultName}>{student.name}</Text>
                  <Text style={styles.searchResultMeta}>Class {student.class}-{student.section} • Roll {student.roll_number}</Text>
                </TouchableOpacity>
              ))}
            </View>
          )}
          ...
          {showForm && selectedStudent && (
            <View style={styles.tcForm}>
              ...
              <Text style={styles.tcFormMeta}>Class {selectedStudent.class}-{selectedStudent.section} • Adm: {selectedStudent.admission_date || 'N/A'}</Text>

              <Text style={styles.fieldLabel}>Leaving Date *</Text>
              <TextInput style={styles.input} placeholder="YYYY-MM-DD" placeholderTextColor={COLORS.textSecondary} value={tcForm.leaving_date} onChangeText={v => setTcForm({ ...tcForm, leaving_date: v })} />

              <Text style={styles.fieldLabel}>Reason for Leaving *</Text>
              <TextInput style={[styles.input, { height: 80, textAlignVertical: 'top' }]} placeholder="Reason (transfer, migration, etc.)" placeholderTextColor={COLORS.textSecondary} multiline value={tcForm.reason} onChangeText={v => setTcForm({ ...tcForm, reason: v })} />

              <Text style={styles.fieldLabel}>Conduct</Text>
              <View style={styles.conductRow}>
                {CONDUCT_OPTIONS.map(c => (
                  <TouchableOpacity key={c} style={[styles.conductChip, tcForm.conduct === c && styles.conductChipActive]} onPress={() => setTcForm({ ...tcForm, conduct: c })}>
                    <Text style={[styles.conductChipText, tcForm.conduct === c && styles.conductChipTextActive]}>{c}</Text>
                  </TouchableOpacity>
                ))}
              </View>

              <TouchableOpacity style={[styles.generateBtn, saving && { opacity: 0.6 }]} onPress={handleGenerateTC} disabled={saving} activeOpacity={0.8}>
              ...
              <TouchableOpacity style={styles.cancelFormBtn} onPress={() => { setShowForm(false); setSelectedStudent(null); setSearchQuery(''); }}>
          ...
          <View style={{ flex: 1, backgroundColor: COLORS.background }}>
            ...
            <ScrollView contentContainerStyle={{ padding: 16 }}>
              <View style={styles.tcDetailCard}>
                ...
                <View style={[styles.tcDetailStatus, { backgroundColor: selectedTC.status === 'collected' ? COLORS.success + '18' : COLORS.warning + '18' }]}>
                  <Text style={[styles.tcDetailStatusText, { color: selectedTC.status === 'collected' ? COLORS.success : COLORS.warning }]}>
              ...
              {selectedTC.status === 'issued' && (
                <TouchableOpacity style={styles.collectedBtn} onPress={() => handleMarkCollected(selectedTC)} activeOpacity={0.8}>
