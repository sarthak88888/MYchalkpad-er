              ...
              <>
                <View style={styles.childCard}>
                  ...
                  <View style={styles.childInfo}>
                    ...
                    <Text style={styles.childClass}>Class {selectedChild.class}-{selectedChild.section} • Roll {selectedChild.roll_number}</Text>
                    {lastMarked && <Text style={styles.lastMarked}>Last attendance: {lastMarked}</Text>}
                  </View>
                </View>
                ...
                <View style={styles.statsRow}>
                  ...
                  <View style={[styles.statBox, { borderColor: pendingFees > 0 ? COLORS.error + '40' : COLORS.success + '40' }]}>
                    <MaterialCommunityIcons name="cash" size={22} color={pendingFees > 0 ? COLORS.error : COLORS.success} />
                    ...
                    <View style={styles.recentDots}>
                      {recentAttendance.slice(0, 7).map((rec, i) => (
                        <View key={i} style={styles.recentDotItem}>
                          <View style={[styles.recentDot, { backgroundColor: getStatusColor(rec.status) }]}>
                            <MaterialCommunityIcons name={getStatusIcon(rec.status) as any} size={14} color="#FFFFFF" />
                          </View>
                          <Text style={styles.recentDotDate}>{rec.date?.slice(8, 10)}</Text>
                        </View>
                      ))}
                    </View>
                    <View style={styles.recentLegend}>
                      {[{ color: COLORS.success, label: 'Present' }, { color: COLORS.error, label: 'Absent' }, { color: COLORS.warning, label: 'Late' }].map((l, i) => (
                ...
                <View style={styles.quickLinks}>
                  {QUICK_LINKS.map((link, i) => (
                    <TouchableOpacity key={i} style={styles.quickLinkCard} onPress={() => router.push(link.route as any)} activeOpacity={0.8}>
                ...
                {pendingFees > 0 && (
                  <TouchableOpacity style={styles.feeAlert} onPress={() => router.push('/parent/fees')} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  ...
  childCard: { backgroundColor: COLORS.primary, borderRadius: 16, padding: 16, flexDirection: 'row', alignItems: 'center', gap: 14, marginBottom: 12 },
  childAvatar: { width: 56, height: 56, borderRadius: 28, backgroundColor: 'rgba(255,255,255,0.2)', justifyContent: 'center', alignItems: 'center' },
  ...
  statBox: { flex: 1, backgroundColor: '#FFFFFF', borderRadius: 12, padding: 16, alignItems: 'center', borderWidth: 1.5, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3 },
  ...
  recentCard: { backgroundColor: '#FFFFFF', borderRadius: 12, padding: 16, marginBottom: 12, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3 },
  ...
  quickLinkCard: { width: '47%', backgroundColor: '#FFFFFF', borderRadius: 12, padding: 16, alignItems: 'center', elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3 },
  ...
  feeAlert: { backgroundColor: COLORS.error, borderRadius: 12, padding: 16, flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 12 },
  feeAlertInfo: { flex: 1 },
  feeAlertTitle: { color: '#FFFFFF', fontSize: 14, fontWeight: '700' },
  feeAlertAmount: { color: 'rgba(255,255,255,0.8)', fontSize: 13, marginTop: 2 },
  attAlert: { backgroundColor: '#FFF1F2', borderRadius: 12, padding: 16, flexDirection: 'row', alignItems: 'flex-start', gap: 10, marginBottom: 12, borderWidth: 1, borderColor: COLORS.error + '30' },
