...
export default function ParentComplaintsScreen() {
  ...
  const [form, setForm] = useState({ subject: '', type: '', description: '' });
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await addDoc(collection(db, 'schools', schoolId, 'complaints'), {
        subject: form.subject.trim(), type: form.type,
        description: form.description.trim(),
        submitted_by_name: parentName || 'Parent',
        submitted_by_phone: parentPhone,
        student_name: childName, student_class: childClass,
        status: 'open', admin_reply: null,
        created_at: new Date().toISOString(), school_id: schoolId,
      });
  ...
  function renderComplaintCard({ item }: { item: Complaint }) {
    ...
    return (
      <TouchableOpacity style={styles.card} onPress={() => { setSelectedComplaint(item); setShowDetailModal(true); }} activeOpacity={0.8}>
  ...
  return (
    <View style={styles.root}>
      <View style={styles.header}>
        ...
        <View style={styles.countBadge}><Text style={styles.countText}>{complaints.length}</Text></View>
      </View>
      ...
      <TouchableOpacity style={styles.fab} onPress={() => { setForm({ subject: '', type: '', description: '' }); setFormErrors({}); setShowNewModal(true); }} activeOpacity={0.85}>
      ...
      <Modal visible={showNewModal} animationType="slide" onRequestClose={() => setShowNewModal(false)}>
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          ...
          <ScrollView style={{ flex: 1, backgroundColor: COLORS.background }} contentContainerStyle={{ padding: 16 }} keyboardShouldPersistTaps="handled">
            ...
            <TextInput style={[styles.input, formErrors.subject ? styles.inputErr : null]} placeholder="Brief subject of your complaint" placeholderTextColor={COLORS.textSecondary} value={form.subject} onChangeText={v => setForm({ ...form, subject: v })} />
            {formErrors.subject ? <Text style={styles.fieldError}>{formErrors.subject}</Text> : null}

            <Text style={styles.fieldLabel}>Type *</Text>
            <View style={styles.typeGrid}>
              {COMPLAINT_TYPES.map(type => (
                <TouchableOpacity key={type} style={[styles.typeChip, form.type === type && styles.typeChipActive]} onPress={() => setForm({ ...form, type })}>
            ...
            <TextInput style={[styles.input, { height: 120, textAlignVertical: 'top' }, formErrors.description ? styles.inputErr : null]} placeholder="Describe the issue in detail (min 20 characters)..." placeholderTextColor={COLORS.textSecondary} multiline value={form.description} onChangeText={v => setForm({ ...form, description: v })} />
            ...
            {children.length > 0 && (
              <View style={styles.childInfoBox}>
                ...
                <Text style={styles.childInfoText}>Complaint will be submitted for {children[0].name} (Class {children[0].class}-{children[0].section})</Text>
              </View>
            )}

            <TouchableOpacity style={[styles.submitBtn, saving && { opacity: 0.6 }]} onPress={handleSubmit} disabled={saving} activeOpacity={0.8}>
          ...
          <View style={{ flex: 1, backgroundColor: COLORS.background }}>
            ...
            <ScrollView contentContainerStyle={{ padding: 16 }}>
              <View style={[styles.statusCard, { backgroundColor: STATUS_COLORS[selectedComplaint.status] + '10', borderColor: STATUS_COLORS[selectedComplaint.status] + '30' }]}>
                ...
                <Text style={[styles.statusCardText, { color: STATUS_COLORS[selectedComplaint.status] }]}>{STATUS_LABELS[selectedComplaint.status]}</Text>
              </View>
              ...
              {selectedComplaint.admin_reply ? (
                ...
                <View style={styles.pendingReplyCard}>
                  ...
                  <Text style={styles.pendingReplyText}>Awaiting response from school. We'll notify you when they reply.</Text>
                </View>
              )}
              <View style={{ height: 40 }} />
            </ScrollView>
