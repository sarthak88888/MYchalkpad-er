    ...
    <View style={styles.root}>
      ...
      {children.length > 1 && (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.childSelector} contentContainerStyle={{ paddingHorizontal: 16, gap: 8, alignItems: 'center', paddingVertical: 8 }}>
          {children.map(c => (
            <TouchableOpacity key={c.id} style={[styles.childChip, selectedChild?.id === c.id && styles.childChipActive]} onPress={() => setSelectedChild(c)}>
              <Text style={[styles.childChipText, selectedChild?.id === c.id && styles.childChipTextActive]}>{c.name.split(' ')[0]}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      )}
            ...
            <View>
              <View style={styles.statsCard}>
                <View style={styles.statsRow}>
                  ...
                  <View style={styles.statItem}>
                    <Text style={[styles.statVal, { color: overallStats.pct >= 75 ? COLORS.success : overallStats.pct >= 60 ? COLORS.warning : COLORS.error }]}>
                      {overallStats.pct}%
                    </Text>
                ...
                <View style={styles.progressBar}>
                  <View style={[styles.progressFill, { width: `${overallStats.pct}%`, backgroundColor: overallStats.pct >= 75 ? COLORS.success : overallStats.pct >= 60 ? COLORS.warning : COLORS.error }]} />
                </View>
                {overallStats.pct < 75 && (
                  <Text style={styles.lowAttWarning}>⚠️ Attendance below 75% — required for examinations</Text>
                )}
              </View>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.monthFilter} contentContainerStyle={{ paddingHorizontal: 16, gap: 8, alignItems: 'center', paddingVertical: 8 }}>
                <TouchableOpacity style={[styles.monthChip, selectedMonth === 'all' && styles.monthChipActive]} onPress={() => setSelectedMonth('all')}>
                ...
                {months.map(m => (
                  <TouchableOpacity key={m} style={[styles.monthChip, selectedMonth === m && styles.monthChipActive]} onPress={() => setSelectedMonth(m)}>
                    <Text style={[styles.monthChipText, selectedMonth === m && styles.monthChipTextActive]}>{getMonthLabel(m)}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
            </View>
...
const styles = StyleSheet.create({
  ...
  statsCard: { backgroundColor: '#FFFFFF', margin: 16, borderRadius: 12, padding: 16, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 3 },
  ...
  recordRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFFFFF', paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: COLORS.border, gap: 12 },
  ...
  recordStatus: { flexDirection: 'row', alignItems: 'center', gap: 6, borderWidth: 1, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 6 },
